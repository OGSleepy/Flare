...
export const Avatar: React.FC<AvatarProps> = ({
  pubkey, size = 44, hasStory = false, storySeen = false, onClick,
}) => {
  const profile = useAppStore(s => s.getProfileCache(pubkey));
      ...
      >
        ...
        >
          {profile?.picture ? (
            ...
            <span className="font-display font-bold text-flare-500" style={{ fontSize: size * 0.38 }}>
              {initial}
            </span>
          )}
        </div>
