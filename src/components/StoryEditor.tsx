...
export const StoryEditor: React.FC<StoryEditorProps> = ({ imageDataUrl, imageBlob, onDiscard, signer }) => {
  ...
  const publish = async () => {
    ...
    if (!sendMode) { toast.error('Choose where to send'); return; }
    setPublishing(true);
    try {
      const finalBlob = await buildFinalBlob();
      const file = new File([finalBlob], 'story.jpg', { type: 'image/jpeg' });
      const upload = await uploadFile(file, signer as any);
      const pool = getPool();
      const now = unixNow();

      if (sendMode === 'story' || sendMode === 'both') {
        const event = await (signer as any).signEvent({
          kind: STORY_KIND, content: caption, created_at: now,
          tags: [['url', upload.url], ['m', file.type], ['x', upload.sha256], ['expiration', String(now + STORY_EXPIRY)],
            ...(caption ? [['alt', caption]] : [])],
        });
        await pool.event(event);
        const story: FlareStory = {
          id: event.id, pubkey, mediaUrl: upload.url, mediaType: 'image',
      ...
      if ((sendMode === 'dm' || sendMode === 'both') && selectedContacts.length > 0) {
        const msg = [caption, upload.url].filter(Boolean).join('\n');
        await Promise.all(selectedContacts.map(pk => sendDM(signer as any, pk, msg).catch(() => {})));
  ...
  const contactList = contacts.slice(0, 50);
    ...
    <div className="flex flex-col h-full bg-black">
      ...
      >
        ...
        <canvas
          ...
          ref={el => {
            (drawCanvasRef as any).current = el;
        ...
        <div className="absolute top-14 right-3 safe-top z-20 flex flex-col gap-3">
            ...
            <button
              ...
              className={`w-11 h-11 rounded-2xl flex items-center justify-center border transition-all ${
                activeTool === id
                  ...
                  : 'bg-black/50 backdrop-blur border-white/20 text-white'
              }`}
            >
              {icon}
            </button>
          ...
          {paths.length > 0 && (
            <button
              ...
              className="w-11 h-11 rounded-2xl bg-black/50 backdrop-blur border border-white/20 flex items-center justify-center text-white"
            >
              <Trash2 size={18} />
            </button>
          )}
      ...
      >
        ...
        <div className="flex gap-2">
          <button
            onClick={() => { setSendMode('story'); setShowSendSheet(false); publish(); }}
            disabled={publishing}
      ...
      {showSendSheet && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative bg-surface rounded-t-3xl px-5 pt-4 animate-slide-up"
            ...
            {(sendMode === 'dm' || sendMode === 'both') && contactList.length > 0 && (
              <div>
                <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2">Send to</p>
                <div className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
                  {contactList.map(contact => {
                    const profile = profiles.get(contact.pubkey);
                    const name = profile?.display_name || profile?.name || shortKey(contact.pubkey);
                    const selected = selectedContacts.includes(contact.pubkey);
                    return (
                      <button key={contact.pubkey}
                        onClick={() => setSelectedContacts(c =>
                          c.includes(contact.pubkey) ? c.filter(p => p !== contact.pubkey) : [...c, contact.pubkey]
                        )}
                        className="flex flex-col items-center gap-1 flex-shrink-0">
                        <div className={`w-12 h-12 rounded-full overflow-hidden border-2 transition-all ${selected ? 'border-flare-500' : 'border-transparent'}`}>
                          {profile?.picture
                            ? <img src={profile.picture} alt={name} className="w-full h-full object-cover" />
                            : <div className="w-full h-full bg-surface-overlay flex items-center justify-center text-flare-500 font-bold">{name[0]}</div>
                          }
                        </div>
                        <span className="text-[10px] text-gray-400 max-w-[48px] truncate">{name}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            <button
              onClick={() => { setShowSendSheet(false); publish(); }}
              disabled={publishing || !sendMode || ((sendMode === 'dm' || sendMode === 'both') && selectedContacts.length === 0 && sendMode !== 'story')}
