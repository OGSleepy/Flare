...
import type { FlareStory, Conversation } from '@/types';
...
export const StoryEditor: React.FC<StoryEditorProps> = ({ imageDataUrl, imageBlob, onDiscard, signer }) => {
  ...
  const contactList = conversations.slice(0, 50);
            ...
            {(sendMode === 'dm' || sendMode === 'both') && contactList.length > 0 && (
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
                  {contactList.map((convo: Conversation) => {
                    const profile = profiles.get(convo.pubkey);
                    const name = profile?.display_name || profile?.name || shortKey(convo.pubkey);
                    const selected = selectedContacts.includes(convo.pubkey);
                    return (
                      <button key={convo.pubkey}
                        onClick={() => setSelectedContacts(c =>
                          c.includes(convo.pubkey) ? c.filter(p => p !== convo.pubkey) : [...c, convo.pubkey]
                        )}
