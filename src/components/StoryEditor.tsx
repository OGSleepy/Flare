...
export const StoryEditor: React.FC<StoryEditorProps> = ({ imageDataUrl, imageBlob, onDiscard, signer }) => {
  ...
  // Detect if media is video based on blob type or URL
  const isVideo = imageBlob.type.startsWith('video/') || imageDataUrl.startsWith('blob:');
  ...
  const publish = async () => {
    ...
    setPublishing(true);
    try {
      // For video: upload raw blob. For image: composite layers onto canvas first.
      const finalBlob = isVideo ? imageBlob : await buildFinalBlob();
      const ext = isVideo ? (imageBlob.type.includes('webm') ? 'webm' : 'mp4') : 'jpg';
      const mime = isVideo ? (imageBlob.type || 'video/mp4') : 'image/jpeg';
  ...
  return (
    <div className="flex flex-col h-full bg-black">
      ...
      >
        ...
        {/* Hide draw/text/sticker tools for video — canvas compositing not supported */}
        {isVideo && (
          <div className="absolute top-14 right-3 safe-top z-20">
            <div className="bg-black/60 backdrop-blur rounded-xl px-3 py-2 border border-white/10">
              <p className="text-white/50 text-xs">Add text after recording</p>
            </div>
          </div>
        )}
