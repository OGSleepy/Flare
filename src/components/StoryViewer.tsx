...
export const StoryViewer: React.FC<StoryViewerProps> = ({ pubkey, onClose, onReply }) => {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setProgress(p => { if (p >= 100) { clearInterval(timerRef.current!); advance(); return 100; } return p + step; });
    }, interval);
    return () => { if (timerRef.current) clearInterval(timerRef.current); };
  }, [idx, paused, showReply, advance]);

  if (!story) return null;

  const handleSendReply = () => {
    if (!reply.trim()) return;
    onReply?.(pubkey, reply.trim());
    setReply(''); setShowReply(false);
    setActiveChat(pubkey); setScreen('chat'); onClose();
  };
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col select-none">
      <div className="absolute top-0 left-0 right-0 z-10 flex gap-1 px-2 pt-3 safe-top">
        {stories.map((s, i) => (
          <div key={s.id} className="flex-1 h-0.5 rounded-full bg-white/25 overflow-hidden">
            <div className="h-full bg-white rounded-full" style={{ width: i < idx ? '100%' : i === idx ? `${progress}%` : '0%', transition: 'none' }} />
          </div>
        ))}
      </div>
      ...
      <div className="absolute inset-0" onPointerDown={() => setPaused(true)} onPointerUp={() => setPaused(false)} onPointerLeave={() => setPaused(false)}>
        {story.mediaType === 'video'
          ? <video src={story.mediaUrl} autoPlay loop muted={false} playsInline className="w-full h-full object-cover" />
          : <img src={story.mediaUrl} alt="" className="w-full h-full object-cover" />
        }
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom,rgba(0,0,0,0.4) 0%,transparent 30%,transparent 60%,rgba(0,0,0,0.6) 100%)' }} />
      </div>

      <div className="absolute inset-0 flex z-10 pointer-events-none">
        <button className="w-1/3 h-full pointer-events-auto"
          onClick={() => { if (idx > 0) { setIdx(i => i - 1); setProgress(0); } else onClose(); }} />
        <div className="flex-1" />
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 safe-bottom px-4 pb-4">
        {showReply ? (
          ...
          <button onClick={() => { setPaused(true); setShowReply(true); }}
