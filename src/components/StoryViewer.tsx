...
interface StoryViewerProps {
  pubkey: string;
  onClose: () => void;
  onReply?: (pubkey: string, message: string, storyMediaUrl: string, storyMediaType: 'image' | 'video', storyCaption?: string) => void;
}
...
export const StoryViewer: React.FC<StoryViewerProps> = ({ pubkey, onClose, onReply }) => {
  ...
  const isMyStory = pubkey === myPubkey;
  ...
  useEffect(() => {
    ...
    setProgress(0);
    const isGif = /\.gif(\?|$)/i.test(story.mediaUrl);
    const duration = isGif ? 15000 : STORY_DURATION; // GIFs get 15s
    const interval = 50;
  ...
  const handleDeleteStory = async () => {
    ...
    try {
      const { getPool, unixNow } = await import('@/lib/nostr');
      const { accountManager } = await import('@/lib/accountManager');
      const signer = accountManager.active?.signer as any;
      ...
      useAppStore.setState(s => ({ stories: s.stories.filter(x => x.id !== story.id) }));
      onCloseRef.current();
    } catch {
      // silent
    } finally {
  ...
  const handleSendReply = async () => {
    ...
    try {
      await onReplyRef.current?.(pubkey, reply.trim(), story.mediaUrl, story.mediaType, story.caption);
      setReply('');
      setShowReply(false);
      ...
      <div className="absolute top-6 left-0 right-0 z-10 flex items-center gap-3 px-4 safe-top mt-4">
        ...
        {isMyStory && (
          <button
            ...
            className="w-9 h-9 rounded-full bg-black/50 backdrop-blur flex items-center justify-center border border-red-500/40 text-red-400 disabled:opacity-40 active:scale-95 transition-transform"
          >
            {deletingStory
