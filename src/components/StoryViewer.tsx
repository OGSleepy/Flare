...
export const StoryViewer: React.FC<StoryViewerProps> = ({ pubkey, onClose, onReply }) => {
  const { stories, profiles, markViewed, viewedStories, setScreen, setActiveChat } = useAppStore(s => ({
    stories: s.stories.filter(x => x.pubkey === pubkey),
    profiles: s.profiles,
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setProgress(p => {
        if (p >= 100) { clearInterval(timerRef.current!); advance(); return 100; }
        return p + step;
      });
    }, interval);

    return () => { if (timerRef.current) clearInterval(timerRef.current); };
  }, [idx, paused, showReply, story]);
