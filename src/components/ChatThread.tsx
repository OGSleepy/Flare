...
export const ChatThread: React.FC<ChatThreadProps> = ({ pubkey, onBack, onSend }) => {
  const { messages, myPubkey, profile } = useAppStore(s => ({
    messages: s.messages.get(pubkey) ?? [],
    myPubkey: s.pubkey,
    profile: s.profiles.get(pubkey),
  }));
    ...
    <div className="flex flex-col h-full bg-[var(--bg)]">
      ...
      <div className="flex-1 scrollable px-4 py-3 space-y-2">
        {messages.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-700 text-xs mt-1">Only you and {name} can read these</p>
          </div>
        )}
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-[75%] px-4 py-2.5 text-sm text-white ${isMine ? 'bubble-sent' : 'bubble-recv'} ${msg.pending ? 'opacity-60' : ''}`}
              >
                <p>{msg.content}</p>
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-t border-[var(--border)] safe-bottom">
        ...
        <button
          ...
          className="w-10 h-10 rounded-full bg-flare-500 flex items-center justify-center disabled:opacity-40 flex-shrink-0 transition-opacity"
        >
