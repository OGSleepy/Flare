...
import {
  Settings, LogOut, Copy, Check, Edit3, Flame, Camera, Lock, ExternalLink,
} from 'lucide-react';
...
export const ProfileScreen: React.FC = () => {
  const { pubkey, profile, stories, logout, setScreen } = useAppStore(s => ({
    pubkey: s.pubkey,
    profile: s.profile,
    stories: s.stories.filter(x => x.pubkey === s.pubkey),
    logout: s.logout,
    ...
    <div className="flex flex-col h-full bg-[var(--bg)] scrollable">
      ...
      <div className="px-5 -mt-12 relative z-10">
        <div className="flex items-end justify-between">
          ...
          <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-[var(--border)] text-sm text-gray-300 hover:text-white transition-colors bg-surface-raised">
            <Edit3 size={14} />
            Edit
          </button>
        </div>
        ...
        <div className="mt-3">
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 mt-2 text-gray-600 hover:text-gray-400 transition-colors"
          >
        ...
        <div className="flex gap-6 mt-4">
          <div className="text-center">
            <p className="font-display font-bold text-white text-lg">{stories.length}</p>
            <p className="text-gray-600 text-xs">Stories</p>
          </div>
          <div className="text-center">
            <div className="flex items-center gap-1 justify-center">
              <Lock size={11} className="text-flare-500" />
              <p className="font-display font-bold text-white text-lg">E2E</p>
            </div>
            <p className="text-gray-600 text-xs">Encrypted</p>
          </div>
          <div className="text-center">
            <p className="font-display font-bold text-white text-lg">24h</p>
            <p className="text-gray-600 text-xs">Ephemeral</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 mt-6">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Flame size={16} className="text-flare-500" />
            <h3 className="font-semibold text-white text-sm">Live Stories</h3>
          </div>
          <button onClick={() => setScreen('camera')}
            className="flex items-center gap-1 text-flare-500 text-xs font-semibold">
            <Camera size={13} /> Add
          </button>
        </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
            {stories.map(s => (
              ...
              >
                ...
                {s.caption && (
                  <p className="absolute bottom-1.5 left-1.5 right-1.5 text-[9px] text-white/80 line-clamp-2 text-center">{s.caption}</p>
                )}
                {/* Expiry indicator */}
                <div className="absolute top-1.5 right-1.5 bg-black/60 backdrop-blur rounded-full px-1.5 py-0.5">
                  <span className="text-[8px] text-white/70">
                    {Math.max(0, Math.floor((s.expires_at - Date.now() / 1000) / 3600))}h
                  </span>
                </div>
              </button>
            ))}
      ...
      <div className="px-5 mt-6">
        <div className="bg-surface rounded-2xl border border-[var(--border)] p-4 flex items-start gap-3">
          <div className="w-9 h-9 rounded-xl bg-surface-overlay flex items-center justify-center flex-shrink-0">
            <Lock size={16} className="text-flare-500" />
          </div>
          <div>
            <p className="text-white text-sm font-semibold">Decentralized Identity</p>
            <p className="text-gray-500 text-xs mt-0.5 leading-relaxed">
              Your account is a Nostr keypair. No server owns it. Stories expire in 24h. DMs are NIP-17 end-to-end encrypted.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Story viewer for my stories */}
      {viewingStory && pubkey && (
        <StoryViewer pubkey={pubkey} onClose={() => setViewingStory(false)} />
      )}
    </div>
  );
};
