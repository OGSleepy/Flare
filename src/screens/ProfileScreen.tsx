...
export const ProfileScreen: React.FC<ProfileScreenProps> = ({ signer }) => {
  ...
  const handleDeleteStory = async (storyId: string) => {
    if (!signer) { toast.error('Not signed in'); return; }
    ...
    try {
      ...
      // Remove directly — bypass setStories so expiry filter doesn't re-run
            ...
            {stories.map(s => (
              <div key={s.id} className="flex-shrink-0 relative">
                ...
                <button
                  ...
                  className="absolute bottom-1.5 left-1.5 w-6 h-6 rounded-full bg-black/70 flex items-center justify-center text-red-400 border border-red-400/30 disabled:opacity-40"
                >
                  <Trash2 size={11} />
                </button>
              </div>
