...
export const ExploreScreen: React.FC = () => {
  const { stories, setStories, profiles, setProfileCache, viewedStories } = useAppStore(s => ({
    stories: s.stories,
  ...
  // Group into columns for masonry
  const col1: FlareStory[] = [];
  const col2: FlareStory[] = [];
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg)]">
      ...
      <div className="flex-1 scrollable pb-24 px-3">
        ...
        {!loading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-gray-500 text-sm">Nothing to explore yet</p>
          </div>
        )}
              ...
              <div key={ci} className="flex-1 flex flex-col gap-2">
                {col.map(story => {
                  ...
                  return (
                    <button
                      key={story.id}
                      onClick={() => setViewingPubkey(story.pubkey)}
                    ...
                    >
                      ...
                      {!seen && (
                        <div className="absolute top-2.5 left-2.5 w-2 h-2 rounded-full bg-flare-500 shadow-lg" />
                      )}
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-2.5">
                        ...
                        {story.caption && (
                          <p className="text-white/70 text-[10px] mt-0.5 line-clamp-2 text-left">{story.caption}</p>
                        )}
                      </div>
                    </button>
                  );
      ...
      {viewingPubkey && (
        <StoryViewer pubkey={viewingPubkey} onClose={() => setViewingPubkey(null)} />
      )}
    </div>
  );
};
