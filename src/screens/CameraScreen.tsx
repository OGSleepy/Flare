...
import { Zap, X, ImagePlus, Loader2, FlipHorizontal } from 'lucide-react';
...
export const CameraScreen: React.FC<CameraScreenProps> = ({ signer }) => {
    ...
    const start = async () => {
      try {
        ...
        if (videoRef.current) {
          ...
          videoRef.current.onloadedmetadata = () => { if (!cancelled) setCamReady(true); };
        }
  ...
  const publish = async () => {
    ...
    setPublishing(true);
    try {
      const file = capturedBlob instanceof File ? capturedBlob : new File([capturedBlob], 'story.jpg', { type: 'image/jpeg' });
      ...
      const event = await (signer as any).signEvent({
        ...
        tags: [['url', upload.url], ['m', file.type], ['x', upload.sha256], ['expiration', String(now + STORY_EXPIRY)], ...(caption ? [['alt', caption]] : [])],
      });
      await pool.event(event);
      const story: FlareStory = { id: event.id, pubkey, mediaUrl: upload.url, mediaType: 'image', caption: caption || undefined, created_at: now, expires_at: now + STORY_EXPIRY, tags: event.tags };
      addStory(story);
      toast.success('Story posted 🔥');
      discard();
    } catch (e: any) { toast.error(e.message ?? 'Failed to post story'); }
    ...
    <div className="relative flex flex-col h-full bg-black overflow-hidden">
      {!captured ? (
        <>
          ...
          {!camReady && <div className="absolute inset-0 flex items-center justify-center"><Loader2 size={32} className="text-white/40 animate-spin" /></div>}
          {shutterAnim && <div className="absolute inset-0 bg-white opacity-60 pointer-events-none z-20" />}
          <div className="absolute top-0 left-0 right-0 z-10 flex justify-end px-5 pt-14 safe-top">
          ...
          <div className="absolute bottom-0 left-0 right-0 z-10 flex items-center justify-around px-10 pb-24 safe-bottom">
            <button onClick={() => fileRef.current?.click()}
              className="w-12 h-12 rounded-xl bg-white/20 backdrop-blur flex items-center justify-center border border-white/20">
        ...
        <div className="flex flex-col h-full">
          <div className="relative flex-1">
            ...
            <div className="absolute bottom-4 left-4 right-4 z-10">
              <input value={caption} onChange={e => setCaption(e.target.value)} placeholder="Add a caption…" maxLength={140}
