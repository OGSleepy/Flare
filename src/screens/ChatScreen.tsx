...
import { fetchProfiles, shortKey, DM_RELAYS } from '@/lib/nostr';
...
export const ChatScreen: React.FC<ChatScreenProps> = ({ signer }) => {
  ...
  // ── DM subscription via NRelay1 directly on DM relays ──────────────────────
  const relaysRef = useRef<NRelay1[]>([]);

  useEffect(() => {
    if (!pubkey || !signer) return;

    // Close any existing relay connections
    relaysRef.current.forEach(r => r.close?.());
    relaysRef.current = [];

    const processedIds = new Set<string>();

    DM_RELAYS.forEach(url => {
      try {
        const relay = new NRelay1(url);
        relaysRef.current.push(relay);

        // Subscribe to gift wraps addressed to us
        const sub = relay.req([{ kinds: [1059], '#p': [pubkey], limit: 100 }]);
        sub.then(async (msgs: AsyncIterable<any>) => {
          for await (const msg of msgs) {
            if (msg[0] !== 'EVENT') continue;
            const wrap: NostrEvent = msg[2];
            if (!wrap?.id || processedIds.has(wrap.id)) continue;
