...
export const ChatScreen: React.FC<ChatScreenProps> = ({ signer }) => {
  const {
    pubkey, activeChatPubkey, setActiveChat, profiles,
    setProfileCache, conversations, setConversations, addMessage, messages,
  } = useAppStore(s => ({
    pubkey: s.pubkey,
    activeChatPubkey: s.activeChatPubkey,
  ...
  useEffect(() => {
    ...
    const sub = (pool as any).req(DM_RELAYS, [{ kinds: [1059], '#p': [pubkey], limit: 100 }]);
    const subscription = sub?.subscribe?.({
      next: async (msg: any) => {
        ...
        const name = profile?.display_name || profile?.name || shortKey(convoKey);
        setConversations(prev => {
          const existing = prev.find(c => c.pubkey === convoKey);
          ...
          if (existing) return prev.map(c => c.pubkey === convoKey ? updated : c);
          return [updated, ...prev].sort((a, b) => b.lastAt - a.lastAt);
        });
      },
    });
      ...
      <div className="px-5 pt-14 pb-3 safe-top">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="w-9 h-9 rounded-full bg-surface flex items-center justify-center text-gray-400 hover:text-white border border-[var(--border)]">
              <Edit size={16} />
            </button>
          </div>
        </div>
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 px-6">
            ...
            <p className="text-gray-500 text-sm text-center">
              No conversations yet.<br />Reply to a story to start chatting!
            </p>
          </div>
        )}
        ...
        <div className="px-4 space-y-1">
          {filtered.map(convo => {
            ...
            return (
              <button
                key={convo.pubkey}
                onClick={() => setActiveChat(convo.pubkey)}
              ...
              >
                <div className="relative flex-shrink-0">
                  ...
                  {convo.unread && (
                    <div className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-flare-500 border-2 border-[var(--bg)]" />
                  )}
                </div>
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center justify-between">
                    <p className={`text-sm truncate ${convo.unread ? 'text-white font-semibold' : 'text-gray-300 font-medium'}`}>
                      {name}
                    </p>
                    <span className={`text-xs flex-shrink-0 ml-2 ${convo.unread ? 'text-flare-500 font-semibold' : 'text-gray-600'}`}>
                      {timeLabel}
                    </span>
                  </div>
                  <p className={`text-xs truncate mt-0.5 ${convo.unread ? 'text-gray-300' : 'text-gray-600'}`}>
                    {convo.lastMessage}
                  </p>
                </div>
              </button>
