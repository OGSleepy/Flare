...
export const ChatScreen: React.FC<ChatScreenProps> = ({ signer }) => {
  ...
  const handleSnapSent = useCallback((mediaUrl: string, mediaType: 'image' | 'video', viewOnce: boolean, caption?: string) => {
    if (!pubkey || !activeChatPubkey) return;
    const now = Math.floor(Date.now() / 1000);
    addMessage(activeChatPubkey, {
      id: `snap-${Date.now()}`,
      senderPubkey: pubkey,
      recipientPubkey: activeChatPubkey,
      content: caption || '',
    ...
    const store = useAppStore.getState();
    const updated: Conversation = { pubkey: activeChatPubkey, lastMessage: viewOnce ? '👻 Snap' : '📸 Snap', lastAt: now, unread: false };
    const exists = store.conversations.find(c => c.pubkey === activeChatPubkey);
    store.setConversations(
      exists
        ? store.conversations.map(c => c.pubkey === activeChatPubkey ? updated : c)
  ...
  }, [pubkey, activeChatPubkey, addMessage]);

  if (activeChatPubkey) {
    return <ChatThread pubkey={activeChatPubkey} onBack={() => setActiveChat(null)} onSend={handleSend} onSnapSent={handleSnapSent} signer={signer} />;
  }
