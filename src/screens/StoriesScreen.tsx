...
import { fetchStories, fetchStoriesFromAuthors, fetchProfiles, shortKey, unixNow } from '@/lib/nostr';
...
export const StoriesScreen: React.FC<StoriesScreenProps> = ({ signer }) => {
  ...
  // Only show stories from yourself + people you follow
  // This is the main NSFW guard — strangers' content never appears here
  const followedPubkeys = pubkey ? new Set([pubkey, ...followList]) : new Set<string>();
  const followedArray = [...followedPubkeys];
  const filteredStories = stories.filter(s => followedPubkeys.size === 0 || followedPubkeys.has(s.pubkey));

  useEffect(() => {
    setLoading(true);
    // Fetch only from followed authors — much safer feed
    const fetcher = followedArray.length > 0
      ? fetchStoriesFromAuthors(followedArray)
      : fetchStories(); // fallback for new accounts with no follows yet
  ...
  const grouped = groupByPubkey(filteredStories);
  ...
  const handleReply = useCallback(async (
    ...
    try {
      // Include the story URL in the DM content so recipient can see context
      const dmContent = storyMediaUrl
        ? `${message}\n\n↩ Replied to your story: ${storyMediaUrl}`
        : message;

      await sendDM(signer as any, toPubkey, dmContent);

      // Store locally with rich media so our chat shows the preview
      ...
      <div className="px-5 pt-14 pb-4 safe-top">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 text-flare-500">
            <Flame size={16} />
            <span className="text-xs font-semibold">{filteredStories.length} live</span>
          </div>
        </div>
      </div>
      ...
      {!loading && filteredStories.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
          ...
          <p className="text-gray-500 text-center text-sm">
            {followList.length === 0
              ? 'Follow people on Nostr to see their stories here.'
              : 'None of your follows have posted a story yet.'}
          </p>
