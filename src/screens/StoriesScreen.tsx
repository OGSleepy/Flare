...
export const StoriesScreen: React.FC<StoriesScreenProps> = ({ signer }) => {
  ...
  const handleReply = async (toPubkey: string, message: string) => {
    if (!signer) return;
    try { await sendDM(signer as any, toPubkey, message); } catch { /* silent */ }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg)]">
      ...
      {viewingPubkey && <StoryViewer pubkey={viewingPubkey} onClose={() => setViewingPubkey(null)} onReply={handleReply} />}
    </div>
  );
};
